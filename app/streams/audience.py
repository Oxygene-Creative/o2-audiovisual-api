...
async def _process_audience(data: list[dict]):
    try:
        ...
        payload = [
            {"bucket": item.get("_source", {}).get(
                "gcp_bucket"), "blob": item.get("_source", {}).get("gcp_blob")}
        ...
        logger.info(
            f"Sending batch request to {SEGMENTATION_GPU_URL}/vad/batch for audience analysis")
        ...
        for idx, result in enumerate(response.json()):
            ...
            logger.info(
                f'audience analysis for {data[idx]["_source"]["source"]["name"]} completed in {time_taken}s')
        ...
        logger.error(
            f"An unexpected error occurred during audience analysis: {e}")
        raise
