...
def add_demographics_slide(prs, data, report_date, title="DEMOGRAPHICS"):
    try:
        ...
        summary.text = data['gender_summary']
        change_text_color(summary, text_color_rgb)
        
        # create pie chart
        sov_chart = slide.placeholders[23]
        sov_chart_colors = [hex_to_rgb(color) for color in data['gender_data']['colors']]
        ...
        print(f"Error creating demographics slide: {e}")
        raise
